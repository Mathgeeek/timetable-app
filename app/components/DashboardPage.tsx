...
// 💡 부모 컴포넌트(page.tsx)로부터 엑셀 파싱 유닛 리스트를 주입받도록 인터페이스 지정
interface DashboardProps {
  units: TeacherUnit[];
}

export default function DashboardPage({ units }: DashboardProps) {
  ...
  // 첫 번째 교사명을 초기값으로 설정하기 위한 유동적 상태 제어
  ...
  const handleDrop = (e: React.DragEvent, day: string, period: number) => {
    ...
    const targetGrade = viewMode === 'CLASS' ? selectedGrade : unit.grade;
    const targetClass = viewMode === 'CLASS' ? selectedClass : unit.classNum;
    const targetTeacher = viewMode === 'CLASS' ? unit.name : selectedTeacher;

    const isTeacherBusy = assignments.some(
      a => a.day === day && a.period === period && a.name === targetTeacher
    );
    if (isTeacherBusy) {
      alert(`${targetTeacher} 선생님은 해당 교시에 이미 다른 반 수업이 배정되어 있습니다!`);
      return;
    }
              ...
              .map((unit) => {
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-xl border bg-slate-900 hover:bg-slate-850 active:scale-95 transition-all duration-200 cursor-grab shadow-md group relative overflow-hidden ${unit.color}`}
                  >
                    <div className="absolute top-0 right-0 bg-slate-800 border-l border-b border-slate-700 px-2.5 py-1 rounded-bl-lg text-xxs font-bold text-slate-300">
                    ...
                    <div className="text-xxs text-slate-500 mt-1">대상: {unit.grade}학년 {unit.classNum}반</div>
                  </div>
                );
              })
          )}
        ...
        <div className="flex-1 bg-slate-900/40 border border-slate-800 rounded-2xl p-6 overflow-auto backdrop-blur-sm shadow-xl flex flex-col">
          <div className="w-full border-collapse flex flex-col flex-1 min-w-3xl">
            ...
            <div className="flex-1 flex flex-col justify-between pt-2">
              {PERIODS.map((period) => (
                <div key={period} className="grid grid-cols-6 items-center py-2 group/row">
                      ...
                      <div key={`${day}-${period}`} className="px-1.5 h-full min-h-20" onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, day, period)}>
                        {placedUnit ? (
                          <div onClick={() => handleCellClick(placedUnit)} className={`w-full h-full rounded-xl border p-2 flex flex-col justify-center items-center text-center cursor-pointer relative bg-slate-900/90 ${placedUnit.color} ${placedUnit.isFixed ? 'ring-2 ring-amber-500 border-amber-500' : ''}`}>
                            {placedUnit.isFixed && <span className="absolute top-1 right-1 text-amber-400 text-xxs font-bold">📌 고정</span>}
                            <span className="text-xs font-bold">{viewMode === 'CLASS' ? `${placedUnit.name} 교사` : `${placedUnit.grade}-${placedUnit.classNum}반`}</span>
                            <span className="text-xxs opacity-80 mt-0.5">{placedUnit.subject}</span>
                          </div>
                        ) : (
